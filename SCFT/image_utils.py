...
def xdog(img, gamma = 0.97, phi = 200, epsilon = 0.1, k = 1.5, sigma = 0.7):
    
    im = array(ImageEnhance.Sharpness(img).enhance(3.0))
...
def appearence_transformation(args, img):
    noise = np.random.uniform(args.noise_lower, args.noise_upper) 
    out = np.array(img) + noise
